...
export default function UploaderScreen() {
  ...
  const handlePdf = async () => {
    ...
    try {
      const result = await fetchGeminiPDF(pdf);
      if (result) {
        clearEvents();
        addEvents(result);
        setStatus('success');
  ...
  const handleText = async () => {
    ...
    try {
      const result = await fetchGeminiText(pastedText);
      if (result) {
        clearEvents();
        addEvents(result);
        setStatus('success');
      ...
      <View style={styles.blobContainer}>
        <Animated.View
          style={[
            styles.blob,
            styles.blob_one,
            { transform: [{ translateY: blobAnim1.interpolate({ inputRange: [0, 1], outputRange: [0, 30] }) }] },
          ]}
        />
        <Animated.View
          style={[
            styles.blob,
            styles.blob_two,
            { transform: [{ translateX: blobAnim2.interpolate({ inputRange: [0, 1], outputRange: [0, -20] }) }] },
          ]}
        />
        <Animated.View
          style={[
            styles.blob,
            styles.blob_three,
            { transform: [{ translateY: blobAnim2.interpolate({ inputRange: [0, 1], outputRange: [0, -30] }) }] },
          ]}
        />
        <Animated.View
          style={[
            styles.blob,
            styles.blob_four,
            { transform: [{ translateX: blobAnim1.interpolate({ inputRange: [0, 1], outputRange: [0, 20] }) }] },
          ]}
        />
      </View>
