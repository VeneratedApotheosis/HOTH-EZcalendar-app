...
export const EmailRow = React.memo(
  ({
    ...
    >
      <View style={[styles.checkbox, isSelected && styles.checkboxSelected]}>{isSelected && <View style={styles.checkBall} />}</View>

      <View style={styles.emailContent}>
        <View style={styles.emailHeader}>
          <Text style={[styles.sender, !email.isRead && styles.senderUnread]} numberOfLines={1}>
            {email.sender}
          </Text>
...
export const SelectedEmailCard = ({ email, onRemove }: { email: EmailData; onRemove: (id: string) => void }) => (
  <View style={styles.selectedCard}>
    <View style={styles.cardTop}>
      <Text style={styles.cardSender} numberOfLines={1}>
        {email.sender}
      </Text>
      <TouchableOpacity onPress={() => onRemove(email.id)} style={styles.closeButton}>
